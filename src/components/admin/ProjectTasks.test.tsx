...
// Phase 35 Plan 05 Task 2 — DASH-20/21/22 on the project-detail Tasks card.
...
describe("ProjectTasks (Phase 35 Plan 05 Task 2)", () => {
  ...
  it("renders `Show completed (N)` when completed tasks exist", () => {
    render(
      <ProjectTasks
        tasks={makeTasks({ active: 1, completed: 3 })}
        projectId="proj-1"
      />,
    );
  ...
  it("clicking the reveal shows completed rows and swaps to `Hide completed`", () => {
    render(
      <ProjectTasks
        tasks={makeTasks({ active: 1, completed: 2 })}
        projectId="proj-1"
      />,
    );
