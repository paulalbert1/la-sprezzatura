...
<task type="auto" tdd="true">
  ...
  <action>
    ...
    Step 7 — Final smoke: `npx vitest run src/lib/sendUpdate src/lib/workOrder src/emails` exits 0 across all three (this matches the `test:email` npm script wired in 45-01-PLAN).
...
<task type="auto">
  ...
  <action>
    ...
    Step 6 — Run the npm-script aliases to confirm wiring from 45-01-PLAN works:
...
<output>
After completion, create `.planning/phases/45-email-foundations/45-05-SUMMARY.md` per `$HOME/.claude/get-shit-done/templates/summary.md`. Capture: snapshot counts (5+/2+ Vitest, 3 Playwright), the `npm run test:visual` runtime, and a note that Phase 46 will swap fixture inputs into the same harness once the templates are migrated.
</output>
