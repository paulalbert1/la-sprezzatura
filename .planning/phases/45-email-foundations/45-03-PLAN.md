...
user_setup:
  - service: "GitHub (separate repo)"
    why: "Standalone Vercel project per CONTEXT D-01 — `sprezza-hub-email-assets` lives outside this codebase."
    actions:
      - task: "Create new repo `sprezza-hub-email-assets` under ~/Dropbox/GitHub/"
  ...
  - service: vercel
    ...
    actions:
      - task: "Create new Vercel project pointed at `sprezza-hub-email-assets`"
        location: "vercel.com → New Project → Import GitHub repo"
      - task: "Add custom domain `email-assets.sprezzahub.com` in project Settings → Domains"
        location: "Vercel Dashboard"
      - task: "Capture screenshot of the Vercel domain showing SSL provisioned + green status"
        location: "Phase summary"
  - service: cloudflare
    why: "Two DNS edits this phase, in two zones: (a) `sprezzahub.com` zone — add CNAME for email-assets.sprezzahub.com (DNS-only / grey cloud); (b) `lasprezz.com` zone — amend SPF TXT to include amazonses.com. lasprezz.com DNS lives at Cloudflare per memory `project_email_migration_status`. sprezzahub.com was just registered — operator must confirm the zone is at Cloudflare (transfer/import if registered elsewhere) BEFORE Task 1 can complete."
    actions:
      - task: "PREREQUISITE: confirm `sprezzahub.com` DNS is hosted at Cloudflare. If registered elsewhere, transfer the zone to Cloudflare first (or update nameservers) — operator step, may take 24-48h to propagate."
        location: "Cloudflare Dashboard → Add a Site → sprezzahub.com (or verify existing zone)"
      - task: "Add CNAME `email-assets` → Vercel-provided target (e.g. cname.vercel-dns.com), Proxy: DNS-only (grey cloud), TTL: Auto"
        location: "Cloudflare Dashboard → sprezzahub.com → DNS → Records"
      ...
      - task: "Capture before/after dig output for SPF + DKIM + DMARC (DKIM/SPF/DMARC all live on lasprezz.com — sender domain unchanged per app-rename decision 2026-04-26)"
        location: "Phase summary"
  - service: resend
...
must_haves:
  truths:
    - "`sprezza-hub-email-assets` repo exists at `~/Dropbox/GitHub/sprezza-hub-email-assets/` with `vercel.json`, `public/wordmark.png`, `public/brand-mark.png`, `README.md`."
    - "Standalone Vercel project for `sprezza-hub-email-assets` is deployed and `email-assets.sprezzahub.com` resolves to it (DNS-only / grey cloud)."
    - "`curl -sI https://email-assets.sprezzahub.com/wordmark.png` returns HTTP 200, `Cache-Control: public, max-age=31536000, immutable`, and NO `Set-Cookie` header (T-V14-03 mitigation)."
  ...
  artifacts:
    - path: "~/Dropbox/GitHub/sprezza-hub-email-assets/"
  ...
  key_links:
    - from: "email-assets.sprezzahub.com (Cloudflare CNAME)"
      to: "Vercel deployment of sprezza-hub-email-assets"
...
1. **EMAIL-10 (asset host):** Stand up the standalone `sprezza-hub-email-assets` Vercel project with cookie-less serving of `wordmark.png` + `brand-mark.png` behind `email-assets.sprezzahub.com` (Cloudflare DNS-only).
...
<interfaces>
<!-- sprezza-hub-email-assets/vercel.json - exact shape -->
```json
{
...
<!-- Cloudflare DNS records to add/edit — TWO ZONES -->

ADD (in `sprezzahub.com` zone — newly registered domain):
  Type:  CNAME
  Name:  email-assets
  ...
  TTL:   Auto
  Note:  If the sprezzahub.com zone is not yet at Cloudflare, the operator must add the site first
         (Cloudflare Dashboard → Add a Site → sprezzahub.com). Nameserver propagation can take 24-48h.

EDIT (in `lasprezz.com` zone — sender domain, unchanged by app rename):
  Type:  TXT
...
<task type="checkpoint:human-action" gate="blocking">
  <name>Task 1: Create the sprezza-hub-email-assets repo, deploy it to Vercel, point email-assets.sprezzahub.com at it</name>
  <files></files>
  <read_first>
    ...
    - .planning/phases/45-email-foundations/45-PATTERNS.md "sprezza-hub-email-assets/" section
  ...
  <action>
    ...
    1. Create the standalone repo locally at `~/Dropbox/GitHub/sprezza-hub-email-assets/` with `public/`, `vercel.json` (per the interfaces block), and a `README.md`. Source the brand wordmark + brand mark PNGs from the studio brand asset library (manual file copy step — confirm with operator if path is unclear before proceeding).
    2. Initialize git, commit the initial assets + vercel.json + README, and run `gh repo create sprezza-hub-email-assets --public --source=. --push` (ask operator to choose `--public` vs `--private` before running; default to `--private` if uncertain since the Vercel deploy URL stays public regardless).
    ...
    4. Once the operator confirms `email-assets.sprezzahub.com` resolves, Claude runs `curl -sI https://email-assets.sprezzahub.com/wordmark.png` to capture the headers and pastes the verbatim output into the phase summary. Claude also runs the same curl against `/brand-mark.png` to confirm the second asset.
  ...
  <how-to-verify>
    ...
    1. **Create the repo locally + on GitHub:**
       ```
       cd ~/Dropbox/GitHub/
       mkdir sprezza-hub-email-assets
       cd sprezza-hub-email-assets
       git init
       mkdir public
       ...
       EOF
       cat > README.md <<'EOF'
       # sprezza-hub-email-assets

       Standalone, cookie-less asset host for Sprezza Hub outbound email.
       Deployed at `email-assets.sprezzahub.com`.
       ...
       gh repo create sprezza-hub-email-assets --public --source=. --push
       ```
       ...
       **Confirm:** `gh repo view sprezza-hub-email-assets` succeeds.

    2. **Connect Vercel to the repo:**
       Browse to https://vercel.com/new, select `sprezza-hub-email-assets`, accept defaults (no framework, root `/`), Deploy.

       **Confirm:** Default Vercel deployment URL (`sprezza-hub-email-assets.vercel.app` or similar) returns HTTP 200 for `/wordmark.png`.

    3. **Add the custom domain:**
       In the Vercel project: Settings → Domains → Add → `email-assets.sprezzahub.com`. Vercel will display the exact CNAME target it expects.
    ...
    4. **Add the Cloudflare CNAME (DNS-only):**
       Cloudflare → **sprezzahub.com** → DNS → Records → Add:
       (If the zone isn't at Cloudflare yet, add it via Cloudflare Dashboard → Add a Site first; nameserver propagation may take 24-48h.)
       - Type: `CNAME`
       - Name: `email-assets`
       ...
       **Confirm:** `dig +short CNAME email-assets.sprezzahub.com` returns the Vercel target after ~1-2 min propagation.

    5. **Wait for Vercel SSL provisioning** (~30 sec to 5 min after DNS resolves):
       Vercel dashboard → Domains → `email-assets.sprezzahub.com` should turn green / "Valid Configuration".

       **Confirm:** `curl -sI https://email-assets.sprezzahub.com/wordmark.png` returns HTTP 200.

    6. **Smoke-test cache headers + cookie-less:**
       ```
       curl -sI https://email-assets.sprezzahub.com/wordmark.png
       ```
       Output must include:
       - `HTTP/2 200`
    ...
    7. **Capture screenshots for phase summary:**
       - Vercel project domain page showing `email-assets.sprezzahub.com` valid + SSL active.
  ...
  <verify>
    <automated>curl -sI https://email-assets.sprezzahub.com/wordmark.png | tee /tmp/45-asset-headers.txt; grep -c '^HTTP/.* 200' /tmp/45-asset-headers.txt; grep -ci 'cache-control: public, max-age=31536000, immutable' /tmp/45-asset-headers.txt; if grep -i '^set-cookie:' /tmp/45-asset-headers.txt; then echo COOKIE_LEAK_FAIL; exit 1; else echo NO_COOKIE_OK; fi</automated>
  </verify>
  <acceptance_criteria>
    - `test -d ~/Dropbox/GitHub/sprezza-hub-email-assets/` exits 0.
    - `test -f ~/Dropbox/GitHub/sprezza-hub-email-assets/vercel.json` exits 0.
    - `test -f ~/Dropbox/GitHub/sprezza-hub-email-assets/public/wordmark.png` exits 0.
    - `test -f ~/Dropbox/GitHub/sprezza-hub-email-assets/public/brand-mark.png` exits 0.
    - `gh repo view sprezza-hub-email-assets --json name -q .name 2>/dev/null` returns `sprezza-hub-email-assets`.
    - `dig +short CNAME email-assets.sprezzahub.com | grep -ic vercel` returns at least 1 (CNAME points at Vercel).
    - `curl -sI https://email-assets.sprezzahub.com/wordmark.png | grep -c '^HTTP/.* 200'` returns 1.
    - `curl -sI https://email-assets.sprezzahub.com/wordmark.png | grep -ic 'cache-control: public, max-age=31536000, immutable'` returns 1.
    - `curl -sI https://email-assets.sprezzahub.com/wordmark.png | grep -ic '^set-cookie:'` returns 0 (T-V14-03 mitigation — NO cookies on the asset host).
    - `curl -sI https://email-assets.sprezzahub.com/brand-mark.png | grep -c '^HTTP/.* 200'` returns 1 (second asset reachable too).
    - Screenshot of Vercel domain panel (showing SSL valid) is attached to the phase summary.
  </acceptance_criteria>
  <done>The standalone repo exists, Vercel deploys it on auto-push, `email-assets.sprezzahub.com` is a working cookie-less host with year-immutable cache headers, and Cloudflare DNS is DNS-only / grey cloud. Brand wordmark + brand mark are reachable.</done>
</task>
...
| Browser → email-assets.sprezzahub.com | Untrusted: anyone on the internet can fetch images. Mitigation: cookie-less origin (no auth state to leak); cached at edge. |
...
| T-V14-03 | Information Disclosure | Asset host leaks cookies / auth state | mitigate | `email-assets.sprezzahub.com` lives on a SEPARATE registrable domain from the sender (`lasprezz.com`) AND from the app portal (TBD `sprezzahub.com` apex / `app.sprezzahub.com`). Different eTLD+1 means cross-site cookie isolation is enforced by browsers regardless of `Domain=` directives. Plus no auth middleware runs on the asset Vercel project. Verified by `curl -sI ... \| grep -ic '^set-cookie'` returning 0 (Task 1). |
...
<verification>
- `curl -sI https://email-assets.sprezzahub.com/wordmark.png` returns 200 + correct `Cache-Control` + no `Set-Cookie`.
...
<success_criteria>
1. `sprezza-hub-email-assets` repo exists, deploys to Vercel, and serves `email-assets.sprezzahub.com/wordmark.png` + `/brand-mark.png` with correct cache headers, no cookies, HTTP 200.
