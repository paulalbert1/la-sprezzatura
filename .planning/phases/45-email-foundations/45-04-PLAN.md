...
<output>
After completion, create `.planning/phases/45-email-foundations/45-04-SUMMARY.md` per `$HOME/.claude/get-shit-done/templates/summary.md`. Capture: the snapshot file size + first 20 lines, the hex-string round-trip assertions, and a note that 45-05-PLAN will reuse `<Scaffold />` for the Playwright visual baseline.
</output>
