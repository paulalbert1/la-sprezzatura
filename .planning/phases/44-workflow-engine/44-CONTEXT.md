...
- **D-06:** **Core deliverable: full template CRUD.** Liz (and future designers) can create unlimited workflow templates, duplicate and modify existing ones, edit any template's phases/milestones/gates, and delete templates that have no active projects referencing them. Templates are not hardcoded — every aspect is configurable through the UI. The Settings page shows a card grid of all templates; `[+ New template]` and `[Duplicate]` actions create new documents; each card navigates to the full editor.
- **D-07:** Full template editor lives at `/admin/settings/workflow-templates/[id]` — a dedicated admin page with phases list, milestone forms, gate pickers, prerequisite selectors, and execution mode toggle. This editor is the primary UX surface for template management.
...
- **D-09:** Three **starter templates** are seeded by a one-time idempotent script (run at deploy time): Full-service residential (spec §5), Design consultation (spec §6), and Staging (spec §6). These are regular `workflowTemplate` documents — Liz can edit, duplicate, or delete them like any other. They exist only to save Liz from building the primary template by hand on day one. The seeding script does NOT constrain what templates can exist.
