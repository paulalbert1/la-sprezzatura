...
- [x] **Phase 48: Smaller Transactional Emails** — 2 reqs (EMAIL-04, EMAIL-05) (completed 2026-05-01)
...
**Plans**: 4 plans, 2 complete (46-01 + 46-04 supersedes 46-02), 1 parked at Checkpoint 1 (46-03 — awaiting re-sequencing to wave 4 per 46-04-CONTEXT D-28)
...
**Plans**: 10 plans across 5 rounds (rounds 2/3/4/5 added after Liz re-tests + round-3/4 code reviews surfaced new gaps):
- [x] 46.1-01-PLAN.md (greeting double-render fix) — Round 1 Wave 1: defensive strip in `src/emails/sendUpdate/Body.tsx` + admin compose UI helper text + tests covering the strip pattern's narrow scope (literal "Hi {Name},?" only, NOT "Hello/Dear/Hey/etc.")
- [x] 46.1-02-PLAN.md (formatDate empty-input guard) — Round 1 Wave 1: one-line guard in `formatDate` at `SendUpdate.tsx:104` + parity guard on `formatLongDate` at `:109` + 8 tests (4 each)
- [x] 46.1-03-PLAN.md (PLAN-AUTHORING-PATTERNS strengthening) — Round 1 Wave 1: docs-only edit to add the bold admonition + Phase 46 example block + checklist nudge
- [x] 46.1-04-PLAN.md (Outlook desktop dark-mode lock — gap-4) — Round 2 Wave 2: `[data-ogsc]`/`[data-ogsb]` rules + MSO conditional inside `EmailShell.tsx <Head>` locking existing palette against Outlook auto-darken; new `EmailShell.test.ts` proves shared-shell propagation to BOTH SendUpdate and WorkOrder; landed at `f867da6` (19 new passing tests, snapshots regenerated additive-only)
- [x] 46.1-05-PLAN.md (left-align table cells — gap-5) — Round 2 Wave 2: 5 `align=` attribute changes in `Milestones.tsx` + `Procurement.tsx` (date, status header/cell, ETA header/cell) + snapshot regen
- [x] 46.1-06-PLAN.md (Outlook-for-Mac dark-mode survival spike — gap-7 research) — Round 3 Wave 1: `46.1-SPIKE-OUTLOOK-MAC.md` research artifact at `af52ca5` -- evaluates 4 candidate techniques across 6 Outlook variants and recommends chosen combination ((a) `<table bgcolor>` body wrapper + (b) `@media (prefers-color-scheme: dark)` block + (d) inline body-attribute paint, ADDITIVE on top of the existing 46.1-04 lock). Archaeology PASS for all three chosen techniques against gap-5 / gap-6 left-alignment invariants. No source code, test, or snapshot changes
- [x] 46.1-07-PLAN.md (procurement layout fix — gap-6) — Round 3 Wave 1 (parallel-safe with 46.1-06): explicit Column widths (Item 60% / Status 22% / ETA 18%) + `verticalAlign: "top"` on body-row Columns + outer `<Section style={{ paddingTop: 16 }}>` wrapper mirroring the ReviewItems pattern + section-scoped HTML inspection tests + snapshot regen
- [x] 46.1-08-PLAN.md (Outlook-for-Mac dark-mode fix — gap-7) — Round 3 Wave 2 (depends-on 46.1-06): apply chosen combination from `46.1-SPIKE-OUTLOOK-MAC.md` to `EmailShell.tsx` `<Head>` and `<Body>` (additive on top of the existing 46.1-04 lock); add 5 new HTML-inspection tests for the new technique markers; SendUpdate and WorkOrder snapshot regen for the new wrapper layer
- [x] 46.1-09-PLAN.md (round-4 BLOCKER + carryover close-out — CR-R3-01 + WR-01..06) — Round 4 Wave 4: applied explicit literal className strings (`pill-${status}` / `cta-${variant}`) on Procurement pill `<span>` + EmailButton CTA so per-class lock rules actually bind (D-15 — HARD verification gate PASSED: 4 `pill-*` + 5 `cta-*` matches in regenerated SendUpdate snapshot); deleted dead `[data-ogsc] .text-*` + `.border-cream-dark` + `*` catch-all in BOTH `[data-ogs*]` block AND `@media` block (D-16/D-17); refactored pill CSS to module-load-time generation from `STATUS_PILL_STYLES` via `buildPillRules()` (D-18 IN-R3-02 — eliminates ~40 lines of mirrored CSS, single source of truth); rewrote `EmailShell.test.ts` hex-substring tests to className-binding shape (29 → 63 tests; all 7 status keys referenced) + rewrote `SendUpdate.test.ts` valign:top + width tests to invariant-shaped per-row count (D-18 #1/#2); folded round-2 carryovers WR-01/02 (stripLeadingGreeting takes clientFirstName + case-insensitive exact-match; Body.test.ts 12 → 19 tests with Mary-Anne/O'Brien/Hi all coverage), WR-03 (export `MAX_LEN` + textarea `maxLength` + character counter — `PersonalNoteParseError(OVER_LIMIT)` unreachable from admin UI), WR-05 (MSO conditional moved out of `<div>`-in-`<head>` into sibling `<style dangerouslySetInnerHTML>`), WR-06 (deleted pill `borderRadius`; audit confirms only EmailButton CTA retains longhand assignment); single atomic commit at `b75ab0b` lands 12 files (10 D-20 + SendUpdate.tsx callsite + WorkOrder snapshot regen); test gate PASSED (zero NEW failures vs baseline; 59 = 59); +40 passing tests; no AI attribution; no new deps; SUMMARY at `.planning/phases/46.1-merge-gate-gap-closure/46.1-09-SUMMARY.md`
- [ ] 46.1-10-PLAN.md (round-5 close-out — CR-R4-01 BLOCKER + WR-R4-01/WR-R4-02/IN-R4-01/IN-R4-02 fold-ins) — Round 5 Wave 5: relocate MSO conditional out of sibling `<style>` element in `<head>` (CSS-parsing context where `<!--` / `-->` consume as CDO/CDC tokens; `mso-color-scheme: light` directive never reached Outlook word-engine) into a `<div dangerouslySetInnerHTML>` child of `<Body>` between `<Preheader>` and `<table bgcolor>` (HTML-parsing context — Option A from round-4 review §CR-R4-01); delete dead `.bg-cream` selector tails from both `[data-ogsb] body` rule (line 122) and `@media body` rule (line 153) — same anti-pattern D-16 already deleted for `.text-*` rules (WR-R4-01); document `clientFirstName ?? ""` coercion trade-off at SendUpdate.tsx:173 + add empty-firstName regression test to Body.test.ts (WR-R4-02 + IN-R4-02 Option C); tighten EmailShell.test.ts MSO substring assertions (5 substring-only `it()` blocks across SendUpdate + WorkOrder render paths) to 2 context-locked `it()` blocks asserting four properties — substring presence + HTML-context placement + `lastStyleClose > lastStyleOpen` (CR-R4-01 regression guard) + directive integrity (IN-R4-01); regenerate SendUpdate + WorkOrder snapshots in lockstep so the broken `</style><style><!--[if mso]>...<![endif]--></style></head>` region disappears; single atomic commit lands 6 files (4 source/test + 2 snapshots); IN-R4-03 (cosmetic snapshot whitespace) intentionally deferred per round-4 review. Round-5 close-out gate: `/gsd-code-review 46.1` must report `findings.blocker == 0` + Liz visual UAT on Outlook for Mac/Windows/web dark + light-mode regression per CONTEXT.md D-21
...
- [x] 48-04-PLAN.md — Wave 4: EMAIL-05 drift-guard integration tests (D-07) — vi.doMock + dynamic-import the route to assert redis.set ex: AND email body change in lockstep
**UI hint**: yes
...
| 48 | v5.3 | 4/4 | Complete   | 2026-05-01 |
