...
- [ ] **Phase 46.1: Merge-Gate Gap Closure** — 0 numbered reqs (gap-closure of Phase 46 merge-gate UAT) — 8 plans across 3 rounds: round 1 wave 1 ✓ (46.1-01..03 gap-1/2/3); round 2 wave 2 ✓ (46.1-04 gap-4 Outlook auto-darken lock at f867da6, 46.1-05 gap-5 left-align cells at 14dabb9); round 3 wave 1 in progress (46.1-06 ✓ at af52ca5 -- gap-7 outlook-mac dark-mode survival spike artifact, 46.1-07 gap-6 procurement layout NEXT); round 3 wave 2 PENDING (46.1-08 gap-7 fix, depends-on 46.1-06)
...
- [ ] 46.1-07-PLAN.md (procurement layout fix — gap-6) — Round 3 Wave 1 (parallel-safe with 46.1-06): explicit Column widths (Item 60% / Status 22% / ETA 18%) + `verticalAlign: "top"` on body-row Columns + outer `<Section style={{ paddingTop: 16 }}>` wrapper mirroring the ReviewItems pattern + section-scoped HTML inspection tests + snapshot regen
- [ ] 46.1-08-PLAN.md (Outlook-for-Mac dark-mode fix — gap-7) — Round 3 Wave 2 (depends-on 46.1-06): apply chosen combination from `46.1-SPIKE-OUTLOOK-MAC.md` to `EmailShell.tsx` `<Head>` and `<Body>` (additive on top of the existing 46.1-04 lock); add 5 new HTML-inspection tests for the new technique markers; SendUpdate and WorkOrder snapshot regen for the new wrapper layer
