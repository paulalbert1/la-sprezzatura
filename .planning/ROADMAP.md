...
- [ ] **Phase 46.1: Merge-Gate Gap Closure** — 0 numbered reqs (gap-closure of Phase 46 merge-gate UAT) — 9/9 plans complete in code across 4 rounds: round 1 wave 1 ✓ (46.1-01..03 gap-1/2/3); round 2 wave 2 ✓ (46.1-04 gap-4 Outlook auto-darken lock at f867da6, 46.1-05 gap-5 left-align cells at 14dabb9); round 3 ✓ (46.1-06 af52ca5 SPIKE, 46.1-07 016c41c gap-6 procurement layout, 46.1-08 9f9cd05 gap-7 outlook-mac fix); round 4 ✓ (46.1-09 b75ab0b CR-R3-01 BLOCKER + WR-01..06 carryovers — className binding on pill `<span>` + EmailButton CTA + dead-rule deletion + `buildPillRules()` CSS-gen + className-binding-shape test rewrite + 6 round-2 WR carryovers in single atomic commit). Round-5 close-out gates: `/gsd-code-review 46.1` must report findings.blocker == 0 + Liz visual UAT on Outlook for Mac/Windows/web dark + light-mode regression per D-21. Phase 46 closes once round-5 visual UAT returns `approved`.
...
**Plans**: 9 plans across 4 rounds (rounds 2/3/4 added after Liz re-tests + round-3 code review surfaced new gaps):
