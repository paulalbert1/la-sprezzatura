...
- [ ] **Phase 46.1: Merge-Gate Gap Closure** — 0 numbered reqs (gap-closure of Phase 46 merge-gate UAT) — 8 plans complete in code across 3 rounds: round 1 wave 1 ✓ (46.1-01..03 gap-1/2/3); round 2 wave 2 ✓ (46.1-04 gap-4 Outlook auto-darken lock at f867da6, 46.1-05 gap-5 left-align cells at 14dabb9); round 3 ✓ (46.1-06 af52ca5 SPIKE, 46.1-07 016c41c gap-6 procurement layout, 46.1-08 9f9cd05 gap-7 outlook-mac fix). **Re-opened for round 4** 2026-04-29 — code review BLOCKER CR-R3-01: dark-mode lock palette in EmailShell targets className hooks that never appear on rendered elements (Tailwind email compiler strips classes, inlines as styles). Round 4 wave 4 in flight: 46.1-09 (BLOCKER + WR carryovers close-out) authored 2026-04-29 -- applies explicit className strings on Procurement pill `<span>` + EmailButton CTA + deletes dead Tailwind text-utility lock rules + refactors pill CSS to module-load-time generation from `STATUS_PILL_STYLES` + rewrites EmailShell.test.ts to className-binding-shape assertions + folds 6 round-2 WR carryovers (stripLeadingGreeting tightening, MAX_LEN export + maxLength enforcement, MSO relocation, pill borderRadius deletion); single atomic commit; round-5 code review + Liz visual UAT gates.
...
- [ ] 46.1-09-PLAN.md (round-4 BLOCKER + carryover close-out — CR-R3-01 + WR-01..06) — Round 4 Wave 4: apply explicit literal className strings (`pill-${status}` / `cta-${variant}`) on Procurement pill `<span>` + EmailButton CTA so per-class lock rules actually bind (D-15); delete dead `[data-ogsc] .text-*` + `.border-cream-dark` + `*` catch-all (D-16/D-17); refactor pill CSS to module-load-time generation from `STATUS_PILL_STYLES` via `buildPillRules()` (D-18 IN-R3-02); rewrite `EmailShell.test.ts` hex-substring tests to className-binding shape + rewrite `SendUpdate.test.ts` valign:top + width tests to invariant-shaped per-row count (D-18 #1/#2); fold round-2 carryovers WR-01/02 (stripLeadingGreeting takes clientFirstName), WR-03 (export MAX_LEN + maxLength enforcement), WR-05 (MSO conditional moved out of `<div>`-in-`<head>`), WR-06 (delete pill borderRadius); single atomic commit per D-20; round-5 code-review + Liz visual UAT gates per D-21
